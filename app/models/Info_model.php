...
class Info_model extends CI_Model {
	...
	public function get_all_with_relation($id =null)
	{
		if($id) 
		{
			$this->db->select('`peserta`.`ID_PESERTA` AS `ID_PESERTA`, `praktikum`.`KODE_PRAKTIKUM` AS `KODE_PRAKTIKUM`, `praktikum`.`NAMA_PRAKTIKUM` AS `NAMA_PRAKTIKUM`, ucase(`peserta`.`KELAS_PESERTA`) AS `KELAS_PESERTA`, `peserta`.`NILAI_PESERTA` AS `NILAI_PESERTA`, `peserta`.`STATUS_PESERTA` AS `STATUS_PESERTA`,`peserta`.`KETERANGAN_PESERTA` AS `KETERANGAN_PESERTA`,`peserta`.PERIODE_PESERTA` AS `PERIODE_PESERTA`, `peserta`.`THNPELAKSANAAN_PESERTA` AS `THNPELAKSANAAN_PESERTA`');
		...
		{
			$this->db->select('`praktikum`.`KODE_PRAKTIKUM` AS `KODE_PRAKTIKUM`, `praktikum`.`NAMA_PRAKTIKUM` AS `NAMA_PRAKTIKUM`, ucase(`peserta`.`KELAS_PESERTA`) AS `KELAS_PESERTA`, `peserta`.`NILAI_PESERTA` AS `NILAI_PESERTA`, `peserta`.`STATUS_PESERTA` AS `STATUS_PESERTA`,`peserta`.`KETERANGAN_PESERTA` AS `KETERANGAN_PESERTA`,`peserta`.PERIODE_PESERTA` AS `PERIODE_PESERTA`, `peserta`.`THNPELAKSANAAN_PESERTA` AS `THNPELAKSANAAN_PESERTA`');
	      	$this->db->from($this->table);
	      	$this->db->join('tbuser', 'NIM_PESERTA = NIM_TBUSER', 'LEFT');
	      	$this->db->join('praktikum', 'PRAK_PESERTA = KODE_PRAKTIKUM', 'LEFT');
	      	$this->db->where('`peserta`.`NIM_PESERTA`', $id);
	...
	public function get_asisten_lab()
	{

		$this->db->select('NAMA_STRLAB, NOMOR_STRLAB, GELAR_DPN_STRLAB, GELAR_STRLAB');
	    $this->db->from("strlab");
	    $this->db->where('JABATAN_STRLAB', 'asisten lab.');
		$result = $this->db->get();
